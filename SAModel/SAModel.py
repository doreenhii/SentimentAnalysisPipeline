...
class SAModel:  
    ...
    def neg_res(self):
        if(self.sent_comp_method == "PARSETREE"):
            #get linear sequence
            print("completeTreeIndices : {} \n self.neg_scope: {}\n".format(self.completeTreeIndices, self.neg_scope))
            print("len(completeTreeIndices) : {} \t \t len(neg_scope): {}".format(len(self.completeTreeIndices), len(self.neg_scope)))
            sequence_indices = TreeToSequenceIndices(self.completeTreeIndices, self.neg_scope)
            
            #debug which words are negated
            #print("Negated words: {}".format(",".join([self.sentence_sequence[i] for i in sequence_indices])))
            #print("Before negating: {}".format(self.valence_tree)) #[DEBUG]
            ...
            for i in range(len(self.neg_scope)):
                ...
                if(self.neg_res_method == "ANTONYM_LOOKUP"):
                    ...
                    #only resolve negation if not -999
                    if(temp[tree_index[-1]] != -999):
                        temp[tree_index[-1]] = negate(temp[tree_index[-1]], self.neg_res_method)

            #print("After negating: {}".format(self.valence_tree))#[DEBUG]


        elif(self.sent_comp_method == "FLAT"):

            #debug which words are negated
            #print("Negated words: {}".format(",".join([self.sentence_sequence[i] for i in self.neg_scope])))
            #print("Before negating: {}".format(self.valence_sequence)) #[DEBUG]
