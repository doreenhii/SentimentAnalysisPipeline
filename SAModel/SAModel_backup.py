...
class SAModel:  
    def __init__(self):
        ...
        # config_data.close()
        config_data_json ={"ROOT_PATH": "C:\\Users\\King\\Desktop\\SAPipeline\\"}
    ...
    def setSentence(self, sent):
        ...
        if(self.model == "NONE" or self.sent_comp_method == "FLAT"):
            self.sentence_sequence = split_sentence_flat(self.sentence)
            ...
            print("self.valence_tree: {}".format(self.valence_tree))
            print("self.sentence_sequence: {}".format(self.sentence_sequence))
            print("self.completeTreeIndices: {}".format(self.completeTreeIndices))


#         if((self.neg_scope_method == "WINDOW" and self.sent_comp_method == "PARSETREE") or
#             (self.neg_scope_method == "NEGTOOL" and self.sent_comp_method == "PARSETREE")):
#             self.sentence_sequence = self.sentence.split(" ")
#             self.completeTreeIndices = get_word_indices(self.sentence_tree)
    ...
    def detectNegScope(self):
        if(self.neg_scope_method == "PARSETREE" and self.sent_comp_method == "PARSETREE"):
            ...
            #print("S_ID[{}]:{}".format(self.sentence_id,self.neg_scope))
            #print(list(enumerate(self.sentence_sequence)))
            ...
            self.neg_scope = resolve_double_negative( list(itertools.chain(*self.NT_neg_scopes[str(self.sentence_id)]["neg_scope"])))
            #print("Sequence NS:",self.neg_scope)
            self.neg_scope = SequenceToTreeIndices(self.completeTreeIndices, self.neg_scope)
        ...
        if(self.model == "NONE" or self.sent_comp_method == "FLAT"):
            ...
            if(self.neg_res_method == "ANTONYM_LOOKUP"):
                for neg_index in self.neg_scope:
                    ...
                    if(antonym_word is not None):
                        antonym_valence = getValence(self.VALENCE_DICT, antonym_word)
                    else:
                        antonym_valence = -999
                        
                    self.valence_sequence[neg_index] = antonym_valence
            ...
            if(self.neg_res_method == "ANTONYM_LOOKUP"):
                ...
                for i in range(len(self.neg_scope)):
                    word = self.sentence_sequence[sequence_indices[i]] 
                    ...
                    if(antonym_word is None):
                        ...
                        antonym_valence = getValence(self.VALENCE_DICT, antonym_word)
                    self.valence_tree[self.neg_scope[i]] = antonym_valence
