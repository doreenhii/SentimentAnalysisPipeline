...
from feature_extraction import extract_features_scope, extract_features_cue, make_complete_labelarray
from utils import *
from file_writing import *

def load_cue_learner(model_path = ""):
    """
    ...
    with open (model_path+"cue_model.pkl",'rb') as f:
        u = pickle._Unpickler(f)
        u.encoding = 'latin1'
        cue_ssvm = u.load()
    
##    cue_ssvm = pickle.load(open('cue_model.pkl','rb'))
    cue_vectorizer = joblib.load(model_path+"cue_vectorizer.pkl")
    cue_lexicon = pickle.load(open(model_path+"cue_lexicon.pkl", "rb"))
    affixal_cue_lexicon = pickle.load(open(model_path+"affixal_cue_lexicon.pkl", "rb"))
    return cue_ssvm, cue_vectorizer, cue_lexicon, affixal_cue_lexicon

def load_scope_learner(model_path = ""):
    """
    Loads the object containing the scope learner from file
    Returns the scope learner and the scope vectorizer
    """
    with open (model_path+"scope_model.pkl", "rb") as f:
        u= pickle._Unpickler(f)
        u.encoding = 'latin1'
        scope_ssvm = u.load()
    scope_vectorizer = joblib.load(model_path+"scope_vectorizer.pkl")
...
def run(mode = "raw", filename = "", path_to_corenlp = "", negtool_directory = ""):
    ...
    if mode == 'raw':
        ...
        elif not path.exists(path_to_corenlp):
            path_to_corenlp = raw_input("ERROR: You specified the wrong path. Please specify the right path:")
        run_corenlp(path_to_corenlp, filename)
        filename = filename + ".conll"

    cue_ssvm, cue_vectorizer, cue_lexicon, affixal_cue_lexicon = load_cue_learner(negtool_directory)
    run_cue_learner(cue_ssvm, cue_vectorizer, cue_lexicon, affixal_cue_lexicon, filename, mode)
    cue_file = filename.split(".")[0] + "_cues.neg"
    scope_ssvm, scope_vectorizer = load_scope_learner(negtool_directory)
    run_scope_learner(scope_ssvm, scope_vectorizer, cue_file, mode)
    remove(cue_file)
...
if __name__ == '__main__':
	...
	argparser.add_argument('-z', '--model_directory', help = "the path to the negtool models", type = str, default = "")
	...
	cue_ssvm, cue_vectorizer, cue_lexicon, affixal_cue_lexicon = load_cue_learner(args.model_directory)
	run_cue_learner(cue_ssvm, cue_vectorizer, cue_lexicon, affixal_cue_lexicon, filename, args.mode)
	cue_file = filename.split(".")[0] + "_cues.neg"
	scope_ssvm, scope_vectorizer = load_scope_learner(args.model_directory)
